...
describe("parser", function()
  describe("Var", function()
    ...
    it("should parse hex numbers", function()
      eq("0x1234DEADBEEF", get_item(get_parsed "var x = 0x1234DEADBEEF", "id", "Number").value)
    end)

    describe("strings", function()
      it("should allow single quote strings", function()
        eq("'a str'", get_item(get_parsed "var x = 'a str'", "id", "StringLiteral").value)
      end)
    ...
    it("should parse const", function()
      eq('"is constant"', get_item(get_parsed 'const x = "is constant"', "id", "StringLiteral").value)
    end)

    it("should parse final", function()
      eq('"is final"', get_item(get_parsed 'const x = "is final"', "id", "StringLiteral").value)
    end)
  ...
  describe("primitive dicts", function()
    for _, primitive in ipairs { "b", "t", "w" } do
      it(string.format("Should handle: %s", primitive), function()
        local parsed = token.parsestring(grammar, make_vim9script(string.format("var %s:glob_var = 1", primitive)))
