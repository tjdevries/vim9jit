...
patterns.V = function(item)
  return lpeg.V(item)
end
patterns.group = lpeg.V
patterns.table_capture = function(item)
  return lpeg.Ct(item)
end
...
patterns.range = function(s, e)
  return lpeg.R(s .. e)
end
patterns.concat = folder(function(a, b)
  return a * b
end)
patterns.branch = folder(function(a, b)
  return a + b
end)
patterns.one_or_more = function(v)
  return v ^ 1
end
patterns.two_or_more = function(v)
  return v ^ 2
end
patterns.any_amount = function(v)
  return v ^ 0
end
patterns.one_or_no = function(v)
  return v ^ -1
end
patterns.look_behind = lpeg.B
patterns.look_ahead = function(v)
  return #v
end
patterns.neg_look_ahead = function(v)
  return -v
end
patterns.neg_look_behind = function(v)
  return -patterns.look_behind(v)
end
patterns.optional_surrounding = function(start, finish, v)
  return patterns.branch(patterns.concat(start, v, finish), v)
end
patterns.optional_surrounding_parenths = function(v)
  return patterns.optional_surrounding(patterns.literal "(", patterns.literal ")", v)
end
...
patterns.end_of_line = patterns.branch(patterns.literal "\r\n", patterns.literal "\r", patterns.literal "\n")
...
patterns.subset_expression = function(tokens, operators)
  return patterns.concat(patterns.one_or_no(operators), tokens, patterns.any_amount(patterns.branch(operators, tokens)))
end
