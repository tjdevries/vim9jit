...
local _assignment = function(mode, match, local_prefix)
  ...
  elseif type_definition == "bool" then
    expression = string.format([[Vim9__Truthy(%s)]], expression)
  end

  return string.format([[%s = %s%s]], prefix, expression, "\n")
end
...
file_mode.CommandBang = function(mode, match)
  return '"!"'
end
...
file_mode.Command = function(mode, match)
  ...
  if command_arguments then
    ...
    return string.format("vim.cmd(string.format(%s, '%s', %s))\n", "[[%s%s]]", command_name, command_bang)
  end
end
...
file_mode.BinaryExpression = function(mode, match)
  ...
  return string.format(format_str, get_result(mode, match[1]), get_result(mode, match[3]))
end
